...
def VerificarFecha(digito):
    ...
    try:
        fecha=input("Enter the date you want to check(aaaa-mm-dd):")
...
def PicoPlaca(placa, hora):
    ...
    if(VerificarFecha(digito)=="No"):
        if ((horaLimInf1<=horaConsulta and horaConsulta<=horaLimSup1) or (horaLimInf2<=horaConsulta and horaConsulta<=horaLimSup2)):
            print("Sorry, you cannot circulate under the indicated parameters.")
        else:
            print("You can circulate under the indicated parameters.")
    else:
        print("You cannot circulate under the indicated parameters.")
    
    
placa=input("Enter the plate you want to check (Ej: 9023 ):")
#fecha=input("Ingrese la fecha la cual desea consultar(dd/mm/aaaa):")
hora=input("Enter the time you want to check(Ej: 19:30 ):")
